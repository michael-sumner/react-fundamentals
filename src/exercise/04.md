...
I've learned to create a component using JSX which is a titlecased function and pass props to it so that the component returns with the JSX element.

Also learned about adding a fragment and that it is essential because the `element` that is being returned into the root can only return 1 thing and that would have to be the fragment if we are to have several components to return, just simply wrap it in the fragment.

We can also pass PropTypes to make sure that the type is checked whilst creating the react component.
